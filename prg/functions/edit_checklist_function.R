...
ReplaceReferenceText <- function(df_target, input_colname, output_colname) {
  ...
  condition <- c(field_head, kFieldText, ref_head) %>%
    str_c(condition_foot) %>%
    paste0(collapse = "|")
  for (i in 1:nrow(output_target)) {
    ...
    output_value <- target %>% map(~ {
      ...
      if (str_detect(target_text, ref_head)) {
        ...
        target_field_id <- temp[ref_index$field_id] %>%
          str_extract(condition_foot) %>%
          as.numeric()
      } else {
        target_field_id <- target_text %>%
          str_remove(kFieldText) %>%
          str_remove(field_head)
      }
      result <- df_reference %>%
        filter(alias_name == alias & field_id == target_field_id) %>%
        .[[kReferenceOutputColname]]
    })
    output_target[i, output_colname] <- output_value %>%
      unique() %>%
      paste0(collapse = "")
  }
...
EditAllocation <- function(allocation) {
  ...
  res <- res %>%
    EditOutputColumns(target_columns$allocation) |>
    distinct()
  return(res)
}
...
EditOutputDataList <- function(input_list) {
  ...
  name <- df_sheet_items %>%
    rename(name := !!sym(kSheetItemsKeys$jpname)) %>%
    EditOutputColumns(target_columns$name)
...
GetTargetJsonForChecklist <- function(raw_json_files) {
  ...
  visitJsonFileNamesHead <- visitJsonsNames %>%
    str_remove("_[0-9]+$") %>%
    unique()
  targetFileNameHead <- visitJsonFileNamesHead %>%
    map(~ {
      targetHead <- .
      filecount <- names(visitJsons) %>%
        str_detect(str_c("^", targetHead, "_[0-9]+$")) %>%
        sum()
      if (filecount == 1) {
        return(NULL)
      }
  ...
  notVisitJsonList <- notVisitJsons %>%
    map(~ .[!is.na(.)]) %>%
    discard(~ length(.) == 0)
...
GetAllocationFormulaField <- function(json_files) {
  ...
  allocationFormulaField <- json_files %>%
    map(~ {
      json <- .$rawJson
  ...
  formulaFieldList <- allocationFormulaField$formula_field %>% map(~ {
    ...
    replaceStrList <- target %>% map(~ {
      temp <- .
      sheetAndFieldList <- temp %>%
        str_remove(kCondStr1) %>%
        str_remove(kCondStr2) %>%
        str_remove_all("\\s") %>%
        str_remove_all("'") %>%
        str_split(",")
  ...
  allocationFormulaFieldText <- allocationFormulaField %>%
    group_by(sheet_id) %>%
