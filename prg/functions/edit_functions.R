    ...
    if (target != kSheetItems & is.data.frame(input_list[[df_name]])) {
      if (nrow(input_list[[df_name]]) > 0) {
        ...
        input_list[[df_name]] <- input_list[[df_name]] %>%
          JoinJpnameAndAliasName(df_sheet_items) %>%
          select(all_of(kNames), everything())
      }
    }
  }
  return(input_list)
...
GetListCdiscSheetConfig <- function(json_files) {
  cdisc_sheet_config_list <- json_files %>%
    map(~ {
...
GetDfFlipFlops <- function(json_files) {
  temp_df_flip_flops <- json_files %>%
    map(~ {
      ...
      df_field_label <- field_items %>%
        select(any_of(kNameAndLabelList)) %>%
      ...
      flip_flops <- flip_flops %>%
        RemoveListElements("codes") %>%
        RemoveListElements("fields") %>%
  ...
  df_flip_flops <- temp_df_flip_flops %>%
    compact() %>%
    bind_rows()
...
GetDfAllocations <- function(json_files) {
  ...
  res <- json_files %>% map_df(~ {
    ...
    others <- flatten_json$allocation %>%
      RemoveListElements(c(kGroups)) %>%
      data.frame()
    allocation <- groupsAndAllocatees %>%
      cbind(others) %>%
      select(any_of(kNamesAndSheetIdAndId), everything())
    return(allocation)
  })
  return(res)
}
GetDfOptions <- function(json_files) {
  ...
  df_option <- json_files %>% map_df(~ {
    ...
    option_values <- field_items[[kOption.values]] %>%
      list_rbind() %>%
      distinct() %>%
      rename_with(~ paste0(kOption.values, "_", .x), everything())
    res <- df_option %>%
      filter(!is.na(!!sym(kFieldItemsKeys$option.id))) %>%
      distinct() %>%
...
JoinJpnameAndAliasName <- function(target, sheet_items) {
  df_sheet_items <- sheet_items %>%
    select(kSheetItemsKeys %>% unlist()) %>%
    rename(sheet_id = id)
