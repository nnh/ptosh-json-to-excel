...
ReplaceItemVisitSheetName <- function(check_group, alias_name_columnName = kAliasNameJapaneseColumnName) {
...
CheckIdenticalItemVisitList <- function(item_visit_by_group_list) {
    res <- item_visit_by_group_list %>% map(~ {
        base_tibble <- .x[[1]] %>% ReplaceItemVisitSheetName(.)
        if (length(.x) == 1) {
            return(base_tibble)
        }
        rest_tibbles <- .x[-1]
        res <- base_tibble
        identical_tibble_1 <- base_tibble %>% select(-all_of(kAliasNameJapaneseColumnName))
        for (i in seq_along(rest_tibbles)) {
            identical_tibble_2 <- rest_tibbles[[i]] %>%
                ReplaceItemVisitSheetName(.) %>%
                select(-all_of(kAliasNameJapaneseColumnName))
            if (identical(identical_tibble_1, identical_tibble_2)) {
                alias_name <- rest_tibbles[[i]][[kAliasNameJapaneseColumnName]] %>%
                    unlist() %>%
                    unique()
                res[[kAliasNameJapaneseColumnName]] <- str_c(
                    res[[kAliasNameJapaneseColumnName]], ", ", alias_name
                )
            } else {
                print(str_c("グループ", base_tibble[["group"]][1], "のシート", base_tibble[[kAliasNameJapaneseColumnName]][1], "は、前のシートと異なる項目があります。\n"))
...
GetItemVisitByGroupList <- function(item_visit_by_group) {
    ...
    res <- item_visit_groups %>%
        map(~ {
            ...
            tibble_item_visit <- item_visit_by_group %>%
                filter(group == !!group)
            alias_name_list <- tibble_item_visit[[kAliasNameJapaneseColumnName]] %>%
                unique()
            tibble_item_visit_by_alias_name <- alias_name_list %>% map(~ tibble_item_visit %>%
                filter(.data[[kAliasNameJapaneseColumnName]] == .x))
...
EditItemVisit <- function(item_visit) {
    ...
    item_visit_by_group_list <- GetItemVisitByGroupList(item_visit_by_group)
    unique_item_visit <- CheckIdenticalItemVisitList(item_visit_by_group_list)
    item_visit_tibble <- unique_item_visit %>% bind_rows()
    return(item_visit_tibble)
}
