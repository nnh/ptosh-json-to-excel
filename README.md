...
10. `source` をクリックし、 画面左下の console に「処理が終了しました。」と出るまでお待ちください。  
処理の途中で「パブリックネットワークとプライベートネットワークにこのアプリのアクセスを許可しますか？」という Windows セキュリティのポップアップが出た場合は、「キャンセル」をクリックして下さい。  
「パッケージ ‘tidyverse’ はバージョン x.x.x の R の下で造られました」という警告メッセージが出力されることがあります。作成されたエクセルファイルに問題がなければ無視してください。   
