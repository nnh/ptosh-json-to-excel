...
for (row in 1:nrow(df_item_sheet)) {
  for (col in 1:ncol(df_item_sheet)) {
    if (is.na(df_item_sheet[row, col]) &&
      is.na(df_item_json[row, col])) {
      next
    }
    if (df_item_json[row, col] != df_item_sheet[row, col]) {
      stop(str_c(
        "item sheet and item json are not equal at row: ", row, ", col: ", col,
        ", sheet: ", df_item_sheet[row, col], ", json: ", df_item_json[row, col]
      ))
    }
  }
}
