...
GetValidatorDates <- function(fieldItem) {
    validators <- fieldItem %>%
        map(~ {
            field_item <- .x
            if (is.null(field_item$validator$date)) {
                return(NA)
            }
            if (length(field_item$validator$date) == 0) {
                return(NA)
            }
            date_before <- if (!is.null(field_item$validator$date$validate_date_before_or_equal_to) &&
                !is.na(field_item$validator$date$validate_date_before_or_equal_to)) {
                field_item$validator$date$validate_date_before_or_equal_to
            } else {
                NA
            }
            date_after <- if (!is.null(field_item$validator$date$validate_date_after_or_equal_to) &&
                !is.na(field_item$validator$date$validate_date_after_or_equal_to)) {
                field_item$validator$date$validate_date_after_or_equal_to
            } else {
                NA
            }
            return(list(date_before = date_before, date_after = date_after, id = field_item$id, sheet_id = field_item$sheet_id, name = field_item$name, label = field_item$label))
        }) %>%
